...
def enhance_streaming_onnx(wav: np.ndarray, sess, win=20480, hop=10240):
    """
    wav: mono float32, [-1,1]; sess: ORT session cu input 'noisy' [1,1,T]
    """
    assert wav.ndim == 1
    frames = []
    for (s, e) in ola_frames(len(wav), win, hop):
        chunk = wav[s:e]
        if len(chunk) < win:
            pad = np.zeros(win, dtype=wav.dtype); pad[:len(chunk)] = chunk
            chunk = pad
        x = chunk[None, None, :].astype(np.float32)
        y = sess.run(["enhanced"], {"noisy": x})[0][0,0]  # [1,1,T] -> [T]
        frames.append(y[:len(chunk)])  # tăiem dacă e padding
