...
def _read_list(p): return Path(p).read_text().strip().splitlines()
...
def make_calib_loader(pairs, sr=16000, secs=2.0):
    ...
    def gen():
        for c,_ in pairs:
            ...
            x = _to_mono(x)
            if fs != sr: x = _resample_linear(x, fs, sr)
...
def quantize_onnx_static(
    ...
    opts = {"quant_format": QuantFormat.QDQ,
...
def export_qat_from_ckpt(ckpt, config, out_model, epochs=3, seed=404):
    ...
    def load_factory():
        ...
        for mod, name in candidates:
            try:
                m = import_module(mod); return getattr(m, name)
            except Exception: pass
        raise ImportError("Nu am găsit un factory pentru model.")
    def construct_model(cfg_path):
        cfg = None
        if cfg_path: 
            import yaml; cfg = yaml.safe_load(Path(cfg_path).read_text())
        fn = load_factory()
        ...
        try:
            ...
            sig = signature(fn)
            if cfg and len(sig.parameters)==1: return fn(cfg)
            return fn(**cfg) if cfg else fn()
        except Exception:
            return fn()
    def export_to_onnx(model, out_path):
        dummy = torch.randn(1,1,32000)
        torch.onnx.export(model, dummy, out_path, opset_version=18,
    ...
    if args.static:
        if args.weight_only:
            ...
            quantize_onnx_weight_only(
                args.model, args.out, keep_nodes=tuple(keep),
        ...
        quantize_onnx_static(
            args.model, args.out, pairs, keep_nodes=tuple(keep),
            method=args.method, percentile=args.percentile,
            per_channel=args.per_channel_weights,
